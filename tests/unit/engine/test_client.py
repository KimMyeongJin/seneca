...
class TestSenecaClient(TestExecutor):
    ...
    @mock.patch("seneca.engine.client.NUM_CACHES", 2)
    def test_publish_tx_then_use_that_tx_with_one_sb_builder_with_conflict(self):
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        client = SenecaClient(sbb_idx=0, num_sbb=1, loop=loop)
        client.metering = False

        contract_name = 'new_stubucks'
        input_hash1 = '1' * 64
        input_hash2 = '2' * 64
        input_hash3 = '3' * 64
        input_hash4 = '4' * 64

        with open(os.path.join(os.path.dirname(SENECA_PATH), 'test_contracts', 'new_stubucks.sen.py'), 'r') as f:
            code = f.read()

        # print("got contract code {}".format(code))
        publish_tx = MockPublishTransaction(sender=MINT_WALLETS['stu'], contract_name=contract_name, contract_code=code)
        special_mint_tx = MockContractTransaction(sender='435e3264395e24eb37a0eb6c322421e701dc332db45536d25eac67924b9321aa',
                                              contract_name=contract_name, func_name='mint',
                                              to='324ee2e3544a8853a3c5a0ef0946b929aa488cbe7e7ee31a0fef9585ce398502',
                                              amount=3)

        tx1_set = self._gen_random_contracts(num=7) + [publish_tx]
        tx2_set = self._gen_random_contracts(num=7) + [special_mint_tx]
        tx3_set = []
        tx4_set = self._gen_random_contracts(num=10)
        ...
        async def _wait_for_things_to_finish():
            ...
            await asyncio.sleep(1)
            log.notice("Tester done waiting")
            client.concurrency = False
            balances = client.get_resource('new_stubucks', 'balances')
            self.assertEqual(balances['324ee2e3544a8853a3c5a0ef0946b929aa488cbe7e7ee31a0fef9585ce398502'], Decimal('3000000'))
